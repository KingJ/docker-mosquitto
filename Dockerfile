...
COPY run.sh /
RUN buildDeps='git alpine-sdk openssl-dev libwebsockets-dev c-ares-dev util-linux-dev hiredis-dev curl-dev libxslt docbook-xsl mariadb-dev mariadb-client-libs'; \
    chmod +x /run.sh && \
